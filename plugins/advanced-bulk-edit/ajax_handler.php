		...
		if($wherenotin !== "" || $catsquery !== "")
		{
			...
			$query ="INSERT INTO {$temptable}(
				...
				$query ="INSERT INTO {$temptable}(
					...
					FROM {$posts} p1
					WHERE p1.ID IN (SELECT post_parent FROM {$temptable} WHERE type=1) AND (p1.post_type='{$post_type}') AND (p1.post_status IN ('publish','draft','private','pending')){$idlimitquery} ORDER BY p1.ID ASC {$limitquery})";
		}	
