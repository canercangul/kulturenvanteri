...
GeoMashup.addAction( 'loadedMap', function ( properties, mxn ) {
	...
	// Add Scale	
	L.control.scale({position:'bottomright'}).addTo(leaflet_map);
	...
	//lc.start(); // https://stackoverflow.com/questions/42940933/html-code-for-using-locate-via-onclick
	//lc.stopFollowing();
...
GeoMashup.createMap = function( container, opts ) {
	...
	this.spinner_div.innerHTML = '<div id="gm-loading-icon" style="-moz-user-select: none; z-index: 100; position: absolute; left: ' + (jQuery( this.container ).width() / 2 ) + 'px; top: ' + (jQuery( this.container ).height() / 2 ) + 'px;">' + '</div>';
	...
	if ( opts.map_content === 'single' ) {
		if ( opts.object_data && opts.object_data.objects.length && !opts.load_kml ) {
			...
			if ( !marker_opts.image ) {
				marker_opts = this.colorIcon( 'glow' );
...
mxn.register('leaflet', {
	...
	Mapstraction: {
		...
		init: function(element, api) {
			...
			this.osm_tile = {
				name: 'Osm',
				attribution: 'Kültür Envanteri &mdash; &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a>',
